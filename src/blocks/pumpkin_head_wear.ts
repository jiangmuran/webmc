...
// Wiki (minecraft.wiki/w/Mob_Head): canonical head IDs use the FULL
// mob name. The wither skeleton's head is `wither_skeleton_skull`,
// not `wither_skull` — that latter ID is the wither boss's projectile
// type, not a wearable item. Old type also missed piglin_head (1.20)
// and didn't handle wither_skeleton in the detection-range table.
...
export interface DetectionQuery {
  wornHead: Headwear;
  mobType: 'enderman' | 'zombie' | 'skeleton' | 'wither_skeleton' | 'creeper' | 'piglin' | 'other';
}
...
export function detectionRangeMultiplier(q: DetectionQuery): number {
  ...
  if (q.wornHead === 'wither_skeleton_skull' && q.mobType === 'wither_skeleton') return 0.5;
  if (q.wornHead === 'creeper_head' && q.mobType === 'creeper') return 0.5;
  if (q.wornHead === 'piglin_head' && q.mobType === 'piglin') return 0.5;
  return 1.0;
}
