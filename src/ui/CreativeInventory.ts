...
const CATEGORY_RULES: readonly { match: RegExp; category: string }[] = [
  { match: /(_log|_planks|_leaves|_sapling|oak|birch|spruce|jungle|acacia|dark_oak|mangrove|cherry|pale_oak)/, category: 'Wood' },
  { match: /(stone|granite|diorite|andesite|deepslate|tuff|basalt|cobblestone|andesite|brick|concrete)/, category: 'Stone' },
  ...
  { match: /(ore|raw_|gold_block|iron_block|diamond_block|emerald_block|lapis_block|netherite_block|redstone_block|copper_block|amethyst)/, category: 'Ore' },
  { match: /(glow|lantern|torch|glowstone|sea_lantern|end_rod|shroomlight|redstone_lamp|campfire|beacon|light_block)/, category: 'Light' },
  { match: /(flower|tulip|rose|daisy|orchid|allium|cornflower|lily|oxeye|dandelion|poppy|azalea|bamboo|cactus|pumpkin|melon|wheat|carrot|potato|beetroot|sugar_cane|mushroom|nether_wart)/, category: 'Plant' },
  { match: /(wool|carpet|banner|bed)/, category: 'Color' },
  { match: /(piston|observer|repeater|comparator|dispenser|dropper|hopper|rail|redstone|lever|button|pressure_plate|trapdoor|fence_gate|tripwire|target|lightning_rod)/, category: 'Redstone' },
  { match: /(sword|pickaxe|axe|shovel|hoe|bow|crossbow|shield|trident|mace|helmet|chestplate|leggings|boots|elytra|totem)/, category: 'Combat' },
  { match: /(beef|porkchop|chicken|mutton|rabbit|cod|salmon|apple|bread|cake|cookie|stew|berry|melon_slice|pie|potion|honey_bottle|milk_bucket)/, category: 'Food' },
];
...
export class CreativeInventory {
  ...
  constructor(parent: HTMLElement, registry: BlockRegistry, private readonly cb: CreativeInventoryCallbacks) {
    ...
    this.search.style.cssText = 'padding:6px 8px;background:rgba(0,0,0,0.4);color:#fff;border:1px solid rgba(255,255,255,0.15);border-radius:4px;outline:none;font:inherit;';
    ...
    this.grid.style.cssText = 'display:grid;grid-template-columns:repeat(auto-fill,minmax(52px,1fr));gap:4px;overflow-y:auto;padding-right:4px;max-height:60vh;';
    ...
    close.style.cssText = 'align-self:flex-end;padding:4px 10px;background:rgba(50,80,110,0.85);color:#fff;border:1px solid rgba(255,255,255,0.18);border-radius:3px;cursor:pointer;font:inherit;font-size:12px;';
  ...
  private buildTabs(): void {
    ...
    const order = ['All', 'Wood', 'Stone', 'Natural', 'Ore', 'Fluid', 'Plant', 'Color', 'Light', 'Redstone', 'Combat', 'Food', 'Misc'];
    ...
    for (const c of sorted) {
      ...
      btn.textContent = c;
      btn.dataset['cat'] = c;
      btn.style.cssText = 'padding:4px 9px;background:rgba(50,80,110,0.45);color:#fff;border:1px solid rgba(255,255,255,0.12);border-radius:3px;cursor:pointer;font:inherit;font-size:11px;';
